...
public class ProductService {
	...
	public Mono<Product> save(ProductDto productDto) {		
		Mono<Boolean> isExist= this.productRepository.findByName(productDto.getName())
								.hasElement();		
	...
	public Mono<Product> update(int id, ProductDto product) {
	...
	public Mono<Void> delete(Integer id) {
		...
		return productId.flatMap(exist-> 
				exist.booleanValue() ? 
						productRepository.deleteById(id) : Mono.error(new CustomException(HttpStatus.NOT_FOUND,NF_MESSAGE)) 
				);
